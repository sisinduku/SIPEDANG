...
class Reservasi extends CI_Controller {
	...
	public function list_reservasi($statusReservasi = "pending") {
		...
		if ($statusReservasi == "approved") {
			...
			$data['listReservasi'] =
			$this->Datareservasi->get_kegiatan();
			$data['tampil_tombol_cetak'] = 1;
			$data['listTag'] = $statusReservasi;
		} else {
		...
		if ($outputType=='pdf') {
			$this->generate_pdf_arsip($data['listReservasi'], $data['pageTitle']);
		} else { // Output HTML
			$data['listKategori'] = $this::$listKategori;
	...
	private function generate_pdf_arsip($listReservasi, $listTitle, $outputFile = null) {
		...
		$pdf->SetFont('Times','',12);
		$pdf->setHeaders(array('No.','Tanggal Peminjaman','Kegiatan','Penyelenggara'));
		$pdf->SetWidths(array(10,70,100,60));
		$cntItem = 0;
		$pdf->headerRow();
		...
		foreach ($listReservasi as $itemReservasi) {
			...
			if (date('Y-m-d', $unixTimeMulai) == date('Y-m-d', $unixTimeSelesai)) {
				...
				$strTanggal  = format_tanggal_formal($unixTimeMulai, true, false);
				$strTanggal .= " - \r\n";
				$strTanggal .= format_tanggal_formal($unixTimeSelesai, true, false);
			}
			$pdf->Row(array(
					$cntItem,
					$strTanggal,
